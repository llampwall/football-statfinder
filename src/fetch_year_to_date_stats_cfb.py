"""Build College Football season-to-date league metrics that mirror the NFL schema."""
...
def _collect_weekly_stats(
    season: int, end_week: int, api_key: str
) -> List[dict]:
    if end_week <= 0:
        return []
    all_games: List[dict] = []
    for week in range(1, end_week + 1):
        try:
            weekly = fetch_cfbd_team_game_stats(season, week, api_key) or []
        except requests.RequestException as exc:  # pragma: no cover - network guard
            raise RuntimeError(f"CFBD games/teams fetch failed for week {week}: {exc}") from exc
        all_games.extend(weekly)
    return all_games
    ...
    for game in games_payload:
        ...
        for idx, team_info in enumerate(parsed):
            ...
            if len(parsed) == 2:
                ...
                opponent_info = next((p for i, p in enumerate(parsed) if i != idx), None)
            ...
            team_total = (
                float(team_info["total"]) if team_info["total"] is not None else float(team_info["rushing"] or 0.0) + float(team_info["passing"] or 0.0)
            )
            opp_total = (
...
def build_league_rows(teams: Dict[str, dict]) -> List[dict]:
...
def _format_for_csv(rows: List[dict]) -> List[dict]:
    formatted: List[dict] = []
    for row in rows:
        csv_row = {
            "Team": row["Team"],
...
def run(season: int, week: int) -> int:
    ...
    if not api_key:
        ...
        print(f"FAIL: CFB league metrics missing CFBD_API_KEY. See {debug_path}")
        return 1

    try:
        fbs_payload = fetch_cfbd_fbs_teams(season, api_key) or []
    except requests.RequestException as exc:
        payload = {
            "season": season,
            "week": week,
            "error": f"FBS teams fetch failed: {exc}",
            "teams_total": 0,
            "teams_with_metrics": 0,
            "covered_fraction": 0.0,
            "fields_required_per_team": REQUIRED_FIELDS,
        ...
        write_debug(debug_path, payload)
        print(f"FAIL: CFB league metrics unable to load FBS teams. See {debug_path}")
        return 1
    ...
    if not fbs_payload:
        payload = {
            ...
            "rank_columns_ok_fraction": 0.0,
            "missing_fields_per_team_sample": {},
        "source_notes": ["cfbd endpoints used: /teams/fbs", ATS_NOTE],
        }
        write_debug(debug_path, payload)
        print(f"FAIL: CFB league metrics no FBS roster returned. See {debug_path}")
        return 1

    teams = _build_team_baseline(fbs_payload)
    end_week = week - 1
    source_notes = [f"cfbd endpoints used: /teams/fbs (season={season})"]
    try:
        games_payload = _collect_weekly_stats(season, end_week, api_key)
        ...
        payload = {
            ...
            "rank_columns_ok_fraction": 0.0,
            "missing_fields_per_team_sample": {},
            "source_notes": source_notes + [f"cfbd endpoint error: /games/teams weeks 1-{end_week}", ATS_NOTE],
        }
        write_debug(debug_path, payload)
        print(f"FAIL: CFB league metrics failed to collect games. See {debug_path}")
        return 1

    if end_week > 0:
        source_notes.append(f"cfbd endpoints used: /games/teams weeks 1-{end_week}")
    else:
        source_notes.append("cfbd endpoints used: /games/teams (skipped, week<=1)")
    source_notes.append(ATS_NOTE)

    _aggregate_games(games_payload, teams)
    league_rows = build_league_rows(teams)
    csv_rows = _format_for_csv(league_rows)
    ...
    if failure:
        print(
            f"FAIL: CFB league metrics coverage insufficient "
    ...
    print(
        f"PASS: CFB league metrics teams {teams_with_metrics}/{teams_total}; "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build CFB season-to-date league metrics.")
