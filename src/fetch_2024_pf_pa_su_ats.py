...
def fetch_pf_pa_su_ats_from_nflverse(season: int = 2024) -> pd.DataFrame:
    """
...
def _read_tr(url: str) -> pd.DataFrame:
    """
    Read TeamRankings table. Prefer a '2024' column; otherwise first numeric column.
    Return DataFrame with columns: Team, val
    """
    ...
    for t in tables:
        if "Team" in t.columns:
            if "2024" in t.columns:
                return t[["Team","2024"]].rename(columns={"2024":"val"})
...
def fetch_tr_metric(name: str) -> pd.Series:
    df = _read_tr(TR_PAGES[name])
...
def emit_2024_league_metrics(pfpa_su_ats: pd.DataFrame, out_csv="final_2024_league_metrics.csv"):
    # Pull TeamRankings metrics
    ry_o  = fetch_tr_metric("RY(O)")
    py_o  = fetch_tr_metric("PY(O)")
    ty_o  = fetch_tr_metric("TY(O)")
    ry_d  = fetch_tr_metric("RY(D)")
    py_d  = fetch_tr_metric("PY(D)")
    ty_d  = fetch_tr_metric("TY(D)")
    to_pg = fetch_tr_metric("TO")
...
# ---------- MAIN ----------
def main():
    # 1) Build PF, PA, SU, ATS for 2024 from nflverse (no scraping headaches)
    pfpa_su_ats = fetch_pf_pa_su_ats_from_nflverse(season=2024)
    pfpa_su_ats.to_csv("final_2024_pf_pa_su_ats.csv", index=False)
    print("Wrote final_2024_pf_pa_su_ats.csv")

    # 2) Build full Caroline-style league metrics CSV for 2024 finals
    emit_2024_league_metrics(pfpa_su_ats)
